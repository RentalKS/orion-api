...
public interface VehicleRepository extends JpaRepository<Vehicle, Long> {
    ...
    @Query("SELECT new com.orion.dto.vehicle.VehicleViewDto(" +
            "v.id, v.registrationNumber, v.model.id, v.year, v.fuelType, " +
            " v.mileage, v.transmission, v.color, v.description, v.image," +
            " v.location.id, v.rateDates.id, v.rateDates.name,v.rateDates.dailyRate," +
            "v.rateDates.weeklyRate,v.rateDates.monthlyRate,v.insurancePolicy.id,v.section.id ) " +
            "FROM Vehicle v " +
            "LEFT JOIN Rental r on r.vehicle.id = v.id " +
            "LEFT JOIN v.section s " +
            "LEFT JOIN Category c on s.category.id = c.id left join c.company cc " +
            ...
            " and c.deletedAt is null and v.location.deletedAt is null" +
            " and r.deletedAt is null and v.user.deletedAt is null " +
            "and s.deletedAt is null and cc.deletedAt is null " +
    ...
    Page<VehicleViewDto> findAllVehicles(@Param("tenantId") Long tenantId,@Param("userIds") List<Long> userIds,
                                         ...
                                         @Param("to") LocalDateTime to, @Param("locationId") Long locationId,
                                         @Param("companyId") Long companyId, @Param("categoryId") Long categoryId,
                                         @Param("sectionId") Long sectionId, @Param("status") VehicleStatus status,

...
public interface CompanyRepository extends JpaRepository<Company, Long> {
    @Query("select new com.orion.dto.company.CompanyDto(c.createdAt, c.createdBy, c.id, c.companyName, c.companyAddress, c.companyEmail, c.companyPhone, c.companyLogo, c.zipCode, c.city, c.state,c.user.id) " +
            "from Company c where c.id = :companyId and c.user.email = :email and c.deletedAt is null and c.tenant.id = :tenantId")
    Optional<CompanyDto> findCompany(@Param("companyId") Long companyId,@Param("email") String email,@Param("tenantId") Long tenantId);

...
public interface ModelRepository extends JpaRepository<Model, Long> {
   @Query("SELECT new com.orion.dto.model.ModelDto(m.id, m.name, m.brand.id, m.type, m.seatingCapacity, m.fuelEfficiency) " +
   ...
   Optional<ModelDto> findModelByIdFromDto(@Param("modelId") Long modelId, @Param("tenantId") Long tenantId);
    ...
    @Query("SELECT new com.orion.dto.model.ModelDto(m.id, m.name, m.brand.id, m.type, m.seatingCapacity, m.fuelEfficiency) " +
            "FROM Model m WHERE m.brand.id = :brandId and m.tenant.id = :tenantId")
    List<ModelDto> findModelsFromThisBrand(@Param("brandId")Long brandId,@Param("tenantId") Long tenantId);
